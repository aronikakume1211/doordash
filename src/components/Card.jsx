...
function Card({ props, lat, lng }) {
    const { id, name, avgRating, totalRatingsString, cloudinaryImageId } = props.info;
    const { lastMileTravel, deliveryTime } = props.info.sla
    let imageBase = `https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_300,h_300,c_fit/${cloudinaryImageId}`;
    return (
        <div class="restaurant_item">
            <Link to={`/restaurant/${id}/${name}/${lat}/${lng}`}>
                <img src={cloudinaryImageId ? imageBase : 'https://img.cdn4dd.com/cdn-cgi/image/fit=contain,format=auto,width=800,quality=50/https://doordash-static.s3.amazonaws.com/media/photosV2/20a0e201-c536-46a3-90e4-0a3d233c92d9-retina-large.jpg'} alt="restaurant image" />
            </Link>
            <h3>{name}</h3>
