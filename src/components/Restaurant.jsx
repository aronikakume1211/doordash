...
function Restaurant() {
    ...
    try {
      await axios.get(api)
        ...
        .then(res => {
          ...
          // console.log(res.data.cards.slice(1));
          res.data.cards.slice(1).forEach(element => {
            console.log(element.card.card.gridElements.infoWithStyle.restaurants);
            element.card.card.gridElements.infoWithStyle.restaurants.forEach(rest => {
              ...
              if (rest.info.id === id) {
                ...
                setLoading(false);
                console.log("restaurant", rest.info);
  ...
  return (
    ...
    <div className='container'>
      ...
      <p> <span> {restaurant?.avgRating < 4.5 ? restaurant?.avgRating : (<b>{restaurant?.avgRating}</b>)} </span>
